...
def test_readme_links_the_v3_release_packaging_documents():
    ...
    assert "Exactly `120/120` independent trials" in readme
    assert "LLM repaired `10/20` cases" in readme
    assert "Hybrid" in readme and "pass@3 `0.45`" in readme
...
def test_v3_architecture_uses_current_algorithm_and_evidence_boundaries():
    ...
    assert "LLM pass@1 / pass@3 | 0.40 / 0.50" in text
    assert "Hybrid pass@1 / pass@3 | 0.30 / 0.45" in text
    assert "RunRecord | 423/423 audit pass" in text
...
def test_v3_demo_uses_real_cli_and_preserves_live_trial_gate():
    ...
    assert "LLM pass@1/pass@3 为 `0.40/0.50`" in text
...
def test_v3_career_pack_exposes_resume_text_and_honest_metrics():
    ...
    assert "LLM pass@1 / pass@3 | 0.40 / 0.50" in text
    assert "Hybrid pass@1 / pass@3 | 0.30 / 0.45" in text
    assert "可直接使用的真实修复结果版" in text
    assert "为什么 Hybrid 反而低于纯 LLM" in text
    assert "不是 Agent repair" in text
...
def test_committed_v3_packaging_verification_hashes_current_artifacts():
    ...
    assert verification["clean_archive"]["candidate_file_count"] > 500
    assert verification["clean_archive"]["release_hygiene"]["status"] == "pass"
    assert verification["clean_archive"]["tests"]["passed"] > 0
    assert verification["clean_archive"]["release_cli"]["offline_status"] == "pass"
    assert verification["clean_archive"]["release_cli"]["complete_status"] == (
        "pending"
    )
    assert verification["clean_archive"]["release_cli"][
        "live_artifact_supplied"
    ] is False
    assert verification["presentation_material"]["live_metrics_complete"] is True
