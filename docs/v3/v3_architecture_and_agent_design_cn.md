...
- **离线发布门通过**：Phase 0-6 的 benchmark、环境、定位、Rule 基线、语义
  校准、记忆和安全证据均通过统一审计。
- **完整发布仍待完成**：20 个案例上的 60 次 LLM 和 60 次 Hybrid 真实模型
  试验尚未执行，因此 live pass@1、pass@3、修复率、成本和延迟仍为
  `null/pending`。

完整状态以
[`phase7_unified_evaluation.json`](phase7_unified_evaluation.json) 为准，任何
简历或演示材料都不能把待测值替换为 Rule、人工修复或旧版 fixture 指标。
...
- Rule/live 状态：[`phase3_offline_verification.json`](phase3_offline_verification.json)
...
- 离线发布验证：[`phase7_offline_verification.json`](phase7_offline_verification.json)
