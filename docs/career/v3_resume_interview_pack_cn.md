...
本材料只引用当前仓库中可审计的 V3 证据。离线 Phase 0-6 和 60 次 LLM +
60 次 Hybrid 真实模型 trial 均已通过统一发布审计，因此可以写入下述冻结的
真实修复指标。任何数字仍必须从统一 artifact 读取，不能从单案例演示或部分
trial 手工估算。
...
Gate 决定能否执行，targeted tests、完整回归和语义 oracle 决定是否算修复。
  ...
  最终在 120 次真实 DeepSeek trial 中，LLM pass@1/pass@3 为 0.40/0.50，
  Hybrid 为 0.30/0.45，所有失败和 blocker 都保留在分母中。
```
...
- 建立 20 个可复现真实 bug、6 个仓库的 BugsInPy benchmark，要求 bug 测试失败、
  ...
  8 类恶意仓库防护和 120-trial 完整性/模型漂移发布审计；真实 LLM 在 20 个
  case 上取得 pass@1/pass@3=0.40/0.50，Hybrid 为 0.30/0.45。
```
...
- 构建真实修复评估契约，按 case-strategy-trial 固定独立性，严格区分 Rule、LLM、
  ...
  使用 Wilson 95% 区间呈现小样本不确定性，拒绝 119/120 的伪完整结果；在
  120 次 live trial 中审计 423 条候选 RunRecord，LLM/Hybrid 三次内分别修复
  10/20 和 9/20 个 case。
```
...
- 实现 bounded-context LLM patch/reflection：只注入 Top-k 函数、必要图邻域、
  ...
  和语义验证链；真实评估中 Reflection 分别在 7/20 个 LLM case 和 3/20 个
  Hybrid case 中恢复至少一次 verified repair。
...
- 设计统一 RunRecord 和发布审计器，记录 provider、精确 model ID、Prompt hash、
  ...
  1412 passed、2 个 Windows symlink fixture skip，release hygiene 5/5。
```
...
| LLM pass@1 / pass@3 | 0.40 / 0.50 | 8/20 trial-1 成功；10/20 三次内成功 |
| Hybrid pass@1 / pass@3 | 0.30 / 0.45 | 6/20 trial-1 成功；9/20 三次内成功 |
| Reflection recovery | LLM 7/20；Hybrid 3/20 | case 级至少一次 Reflection 成功 |
| 成功 trial | LLM 25/60；Hybrid 22/60 | 分别含 9 和 3 个 Reflection trial |
| Live candidate RunRecord | 423/423 audit pass | 1 个 provider timeout 单独分类 |
| Live 成本 | LLM $1.839613；Hybrid $1.006873 | preflight $0.000035 单独统计 |
| 语义校准 | 2/2 human fixes | 不是 Agent repair |
| 记忆 completion | 3/7 -> 7/7 | 受控记忆消融 |
| 安全处理 | 8/8 | 受控 hostile-repo fixture |
| 全量回归 | 见 Phase 7 final verification | 与 live repair rate 分开记录 |

### 正确表达

```text
在 20 个真实 Python bug 上完成 120 次独立 live trial：LLM pass@1/pass@3
为 0.40/0.50，Hybrid 为 0.30/0.45；所有 423 条候选记录、失败和 blocker 均
进入审计，并严格区分 Rule、LLM、Hybrid 与人工校准的成功归因。
```
...
禁止原因：当前 live 指标只覆盖 20 个固定 BugsInPy case；被选定位 profile 的
Graph 权重为 0；Phase 5 的 2/2 是人工 fix 校准，不能写成 Agent repair。

## 5. 可直接使用的真实修复结果版

[`phase7_unified_evaluation.json`](../v3/phase7_unified_evaluation.json) 已为
`status=pass` 且 `claim_eligible=true`，推荐使用以下表述：

```text
- 在 20 个真实 Python bug 上完成 60 次 LLM 与 60 次 Hybrid 独立 trial：
  LLM pass@1/pass@3=0.40/0.50，Hybrid=0.30/0.45；Reflection 分别恢复
  7/20 与 3/20 个 case，模型调用成本合计 $2.846486，并保留 provider、
  environment、syntax、safety、test、semantic 与 generation failure 分母。
```

`$2.846486` 不含单独的 `$0.000035` provider preflight。不要将 candidate-level
成功数、case-level pass@k、provider retry 或并发 wall-clock duration 混为一谈。
...
最后我用 20 个真实 bug、固定 SHA、独立 trial 和统一 RunRecord 做评估。
离线定位、环境、Rule、语义、记忆和安全证据，以及 120 次真实 LLM/Hybrid
trial 都已完成。LLM 三次内修复 10/20 个 case，Hybrid 修复 9/20；报告同时
保留 Wilson 区间、失败 taxonomy、唯一 provider timeout 和 generator 归因。
```
...
### Q5：为什么 Hybrid 反而低于纯 LLM？

这是实验结果，不应该为了叙事而隐藏。LLM pass@3 是 10/20，Hybrid 是 9/20；
两者 Wilson 95% 区间分别为 `[0.2993, 0.7007]` 和 `[0.2582, 0.6579]`，高度
重叠，20-case 样本不足以断言稳定差异。归因记录显示 Hybrid 的 22 个 verified
winning record 全部来自 LLM，Rule 候选没有成功。规则候选会增加搜索与失败
fingerprint，但在当前真实语义缺陷上没有提供有效先验。后续应扩展 case 数、
做配对显著性分析，并改进何时启用 Rule，而不是宣称 Hybrid 一定更强。

### Q6：Reflection recovery 的分母是什么？

发布表中的 `0.35/0.15` 是 case 级指标：20 个 case 中是否至少有一个 trial
通过 Reflection 获得 verified repair，对应 LLM 7/20、Hybrid 3/20。trial 级
观察是 LLM 9/60、Hybrid 3/60；另外直接成功 trial 分别为 16/60 和 19/60。
两种口径回答的问题不同，面试时必须先说分母。

### Q7：为什么 Hybrid 成本更低，但不能说它更高效？

记录的模型费用为 LLM `$1.839613`、Hybrid `$1.006873`，平均每 trial
`$0.0307/$0.0168`。这与本次 provider 的 cache-hit 计费、Prompt 复用和实际
生成长度有关；Hybrid 仍执行了更多 Rule 候选验证，其成功率也略低。成本数字是
当前冻结 provider/pricing snapshot 的实测结果，不是跨模型、跨仓库的普遍效率
结论。候选延迟之和也不能当作并发 wall-clock 时间。

### Q8：一次 provider timeout 会不会让 pass@k 虚高？

不会。`bugsinpy-black-2 / hybrid / trial-2` 的 timeout 保留为一个完整 trial
中的 provider blocker record；retry 仍属于同一 trial identity，不会创建额外
尝试。该 case 的 Hybrid pass@k 分母仍在 20 个 case 中，缺失 trial 也没有被
插补或删除。
...
第一优先级是把 20-case 结果扩展为更多仓库和更难的跨函数/多文件缺陷，并对
LLM 与 Hybrid 做配对显著性和错误聚类分析；第二优先级是根据失败 taxonomy
改进 Rule 触发条件、Reflection 上下文和语义 oracle；第三优先级是把 Windows
进程级防护升级为容器或 Job Object 级网络与资源隔离。现阶段不应优先堆 UI、
多 Agent 角色或无消融证据的向量数据库。
...
4. `docs/v3/phase7_unified_evaluation.json`：真实 pass@k、成本、失败与归因；
5. `docs/v3/v3_ten_minute_demo_guide_cn.md`：现场演示脚本。

面试前逐条确认简历数字仍与统一报告一致；不能只修改简历而不更新 source
artifact、测试和发布审计。
